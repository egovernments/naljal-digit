...
import org.egov.naljalcustomisation.service.CustomisationServiceImpl;
...
import static org.egov.naljalcustomisation.constants.CustomConstants.SEARCH_TYPE_CONNECTION;
...
public class CustomisationQueryBuilder {
    ...
    @Autowired
    @Lazy
    private CustomisationServiceImpl customisationServiceImpl;
...
//			"ORDER BY conn.connectionno";
    ...
    public StringBuilder applyFiltersForPlaneSearch(StringBuilder query, List<Object> preparedStatement, SearchCriteria criteria) {
        if (!StringUtils.isEmpty(criteria.getTenantId())) {
            ...
            if (criteria.getTenantId().equalsIgnoreCase(customisationConfiguration.getStateLevelTenantId())) {
    ...
    public String getSearchQueryString(SearchCriteria criteria, List<Object> preparedStatement,
        ...
        if (!StringUtils.isEmpty(criteria.getMobileNumber()) || !StringUtils.isEmpty(criteria.getPropertyId())) {
            List<Property> propertyList = customServiceUtil.propertySearchOnCriteria(criteria, requestInfo);
        ...
        if(!StringUtils.isEmpty(criteria.getTextSearch()) && !StringUtils.isEmpty(criteria.getTenantId())) {
            WaterConnectionResponse response = customisationServiceImpl.getWCListFuzzySearch(criteria, requestInfo);
...
//		query.append(ORDER_BY_CLAUSE);
    ...
    private String addPaginationWrapper(String query, List<Object> preparedStmtList, SearchCriteria criteria) {
        ...
        Integer limit = customisationConfiguration.getDefaultLimit();
        Integer offset = customisationConfiguration.getDefaultOffset();
...
//				"(select sum(dd.taxamount) as taxamount from egbs_demand_v1 d join egbs_demanddetail_v1 dd on d.id=dd.demandid group by d.consumercode,d.status having d.status ='ACTIVE' and  d.consumercode=conn.connectionno ) as taxamount");
        ...
        if (criteria.getLimit() == null && criteria.getOffset() == null)
            limit = customisationConfiguration.getMaxLimit();

        if (criteria.getLimit() != null && criteria.getLimit() <= customisationConfiguration.getMaxLimit())
